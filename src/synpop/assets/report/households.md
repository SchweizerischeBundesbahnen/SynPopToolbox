...
from synpop.marginals import load_hh_bfs_pred
```
...
def get_hh_structure(pers_df):
    ...
    alone_in_hh = pers_df.groupby('household_id').size().rename('alone_in_hh')
    alone_in_hh = alone_in_hh == 1
    pers_df = pers_df.join(alone_in_hh, on='household_id')

    hh_df = pers_df.groupby('household_id').size().to_frame('members')
...
pc_households_bsf_pred = load_hh_bfs_pred(YEAR)
pc_households_bsf_ist = load_hh_bfs_pred(YEAR_IST)
```
...
if '(Ref)' in str(YEAR_IST):
    bfs_pred = pd.concat([pc_households_bsf_pred], axis=1)
    bfs_pred = bfs_pred.rename(columns={YEAR: f'BFS {YEAR}'})
    plot_colors=['k', 'C0', 'C1']
else:
...
members_per_hh = households.reset_index().groupby('members').size()
pc_members_per_hh = (members_per_hh / members_per_hh.sum() * 100).rename(f'SynPop {YEAR}').iloc[:6]

members_per_hh_ist = households_ist.reset_index().groupby('members').size()
...
ax = hh_sizes_stats.plot.bar(figsize=(10, 6), alpha=0.8, color=plot_colors)
ax.grid(axis='y')
plt.ylabel('% of HH')
...
print(f"Total number of persons with a child at home in {YEAR_IST}: {persons_ist['has_child_in_hh'].sum()} ({persons_ist['has_child_in_hh'].mean():.2%})")
```

```{code-cell} ipython3
:tags: [remove-cell]

print(f"Total number of persons with a child at home in {YEAR}: {persons['has_child_in_hh'].sum()} ({persons['has_child_in_hh'].mean():.2%})")
```

```{code-cell} ipython3
:tags: [remove-cell]

has_child_in_hh_ist = persons_ist.groupby('age').agg({'has_child_in_hh': 'sum', 'zone_id': 'count'})
has_child_in_hh_ist['zone_id'] = has_child_in_hh_ist['zone_id'] - has_child_in_hh_ist['has_child_in_hh']
...
has_child_in_hh = persons.groupby('age').agg({'has_child_in_hh': 'sum', 'zone_id': 'count'})
has_child_in_hh['zone_id'] = has_child_in_hh['zone_id'] - has_child_in_hh['has_child_in_hh']
...
alone_in_hh_ist = persons_ist.groupby('age').agg({'alone_in_hh': 'sum', 'zone_id': 'count'})
alone_in_hh_ist['zone_id'] = alone_in_hh_ist['zone_id'] - alone_in_hh_ist['alone_in_hh']
...
alone_in_hh = persons.groupby('age').agg({'alone_in_hh': 'sum', 'zone_id': 'count'})
alone_in_hh['zone_id'] = alone_in_hh['zone_id'] - alone_in_hh['alone_in_hh']
...
### Scenario Year - SynPop {glue:text}`year`
