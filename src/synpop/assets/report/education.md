...
ORDERED_CATEGORIES = NEW_CATEGORIES["current_edu"]
```
...
%%time
summary_table = fitting_analysis.compute_comparison_summary(persons, persons_ist,
                                                            YEAR, YEAR_IST,
                                                            groupby='current_edu')
```
...
global_pop_growth = '{:.1f} %'.format(100*(persons.shape[0] / persons_ist.shape[0]) - 1)
```
...
```{note}
As reference, the global population growth is: {glue:text}`pop_growth_pct`.
```
