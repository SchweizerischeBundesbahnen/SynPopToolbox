...
def aggregate_synpop(persons_df: pd.DataFrame,
    ...
    aggregates_df[["pop_caravl", "pop_ga", "pop_ht", "pop_va"]] = (
        persons_df.groupby('location_id')[['car_available', 'has_ga', 'has_ht', 'has_va']].sum())
    ...
    aggregates_df["pop_pup_1"] = persons_df.loc[persons_df['current_edu'] == 'pupil_primary',
                                                'location_id'].value_counts().reindex(aggregates_df.index)
    ...
    # Total population
    aggregates_df['pop_total'] = persons_df['location_id'].value_counts().reindex(aggregates_df.index)
    ...
    if age_groups:
        ...
        person_age_group_df = (persons_df.groupby(['location_id', AGE_GROUP])['age'].count().
                               reset_index().pivot(columns=AGE_GROUP, index='location_id'))
    ...
    if businesses_df is not None:
        aggregates_df['jobs_endo'] = businesses_df.groupby('location_id')['jobs_ch'].sum()
        aggregates_df['jobs_total'] = aggregates_df['jobs_endo'] + businesses_df.groupby('location_id')['jobs_cb'].sum()
        aggregates_df['fte_endo'] = businesses_df.groupby('location_id')['fte_ch'].sum()
        aggregates_df['fte_total'] = aggregates_df['fte_endo'] + businesses_df.groupby('location_id')['fte_cb'].sum()
