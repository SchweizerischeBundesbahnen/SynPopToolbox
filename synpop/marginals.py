...
class FSO_PopPredictionsClient:  # FSO if "BSF" in english
    ...
    def load(self, year, szenario_name='Referenzszenario AR-00-2020'):
        ...
        self.pop_by_canton_and_age = (self.pop_by_canton_age_and_nationality
                                      .groupby(['KT_full', 'N_KT', 'age'], observed=True)['pop'].sum()
                                      .reset_index()
                                      )
        ...
        self.pop_by_canton = (self.pop_by_canton_and_age_group.groupby(['KT_full', 'N_KT'], observed=True)
                              .sum()
                              .reset_index()
                              )
    ...
    def _get_population_by_canton_and_age_group(self):
        ...
        df = (df.drop(['Szenario-Variante', 'Jahr', 'Beobachtungseinheit'], axis=1)
              .rename(columns={'Kanton': 'KT_full',
                               ...
                               'DATA': 'pop'})
              .assign(N_KT=lambda x: x['KT_full'].map(cantons.full_names_to_abbreviations))
              ...
              .astype({'KT_full': 'category', 'N_KT': 'category', 'pop': int})
              )
        df = df[['KT_full', 'N_KT', 'age_group', 'pop']]  # change column order
    ...
    def _get_population_by_canton_age_and_nationality(self):
        ...
        df = (df.drop(['Jahr', 'Beobachtungseinheit', 'Geschlecht'], axis=1)
              ...
              .astype({'KT_full': 'category', 'N_KT': 'category', 'age': int, 'is_swiss': bool, 'pop': int})
              )
        df = df[['KT_full', 'N_KT', 'age', 'is_swiss', 'pop']]  # change column order
...
class FSO_ActivePopPredictionsClient:  # FSO if "BSF" in english
    """
    Client to load data from the FSO:
    Szenarien zur Entwicklung der Erwerbsbevölkerung ab 2015 - Erwerbsquote und Erwerbsbevölkerung nach
...
class FSO_PopStatisticsClient:  # FSO if "BSF" in english
    ...
    def load(self, year):
        ...
        self.pop_by_canton_age_group = (self.pop_by_canton_resident_status_and_age_group
                                        .groupby(['KT_full', 'N_KT', 'age_group'], observed=True).agg({'pop': sum})
                                        .reset_index()
                                        )
        ...
        self.pop_by_canton = (self.pop_by_canton_age_group
                              .groupby(['KT_full', 'N_KT'], observed=True).agg({'pop': sum})
                              .reset_index()
                              )
    ...
    def _get_pop_by_canton_resident_status_and_age_group(self):
        ...
        df = df[['KT_full', 'N_KT', 'age_group', 'resident', 'pop']]  # change column order
        return df
