...
def get_businesses_comparison_summary_by_category(df_ist, df_scenario, year_ist, year_scenario, agg_column, query=None):
    ...
    for df, year in zip((df_ist, df_scenario), (year_ist, year_scenario)):
        ...
        stats['total_jobs'] = stats['jobs_ch'] + stats['jobs_cb']
        stats['total_fte'] = stats['fte_ch'] + stats['fte_cb']
...
def save_figure(save_outputs, name, output_dir, dpi, fig_format, bbox_inches, pad_inches):
    if save_outputs:
