...
class SynPopPreprocessor:
    def __init__(self, raw_data_dir: str, output_dir: str, year: int, mobi_zones_shapefile_path: str = None):
        """
        SynPopPreprocessor
        ...
        self.synpop_config = SynPopConfig()  # all pre-processing steps are set in config
    ...
    def preprocess_persons_and_households(self, nrows: int = None, fix_locationid: bool = False,
        ...
        """
        This method loads persons.csv, enriches it by joining to mobi_zones and households (to get location_id). It
        ...
        mobi_zones['KT_full'] = mobi_zones['N_KT'].map(canton_abbreviations)
        ...
        # Defining more derived variables
        ...
        # Fix location_id in 2016 table
        if fix_locationid:
            # The two additional id digits have to be removed
            households['location_id'] = households['location_id'].apply(lambda x: int(str(x)[:-2]))

        # Joining households to geographical zones
        logger.info('Joining households to mobi_zones...')
        households_with_geography = pd.merge(households, mobi_zones, how='left', left_on='location_id', right_on='ID')
        households_with_geography = households_with_geography.drop('ID', axis=1)
        ...
        # Sanity check
        missing_geographies = persons['KT'].isnull().sum()
    ...
    def preprocess_businesses(self, nrows: int = None, fix_locationid: bool = False):
        ...
        if fix_locationid:
            # The two additional id digits have to be removed
            businesses['location_id'] = businesses['location_id'].apply(lambda x: int(str(x)[:-2]))

        # Joining businesses to geographical zones
        logger.info('Joining households to mobi_zones...')
        businesses = pd.merge(businesses, mobi_zones, how='left', left_on='location_id', right_on='ID')
        ...
        # Sanity check
        missing_geographies = businesses['KT'].isnull().sum()
        if missing_geographies > 0:
